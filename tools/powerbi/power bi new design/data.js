...
window.SDC = (function () {
  // ---- Brand tokens -------------------------------------------------
  const C = {
    blue:   '#1574C4', // actual / current
    navy:   '#061D39', // quoted / target / headers
    light:  '#AACEE8', // ETC / forecast
    yellow: '#FFDE51', // overrun / warning
    green:  '#74C415', // on track / positive
    gray:   '#D9D9D9', // inactive / complete
  ...
  // Quoted vs Actual by billing group (from "Quoted and Actual by Billing Group")
  const billingGroups = [
  ...
  // ---- Job master (real job ids / names / customers from PBIX) -----
  // hoursActual / hoursQuoted are project-to-date; parts in $.
  const jobs = [
  ...
  // names pulled from "Employee Utilization" table
  const employeeUtil = [
  ...
  function jobMonthly(job) {
    // deterministic believable 8-month series scaled to the job
  ...
  function jobEmployees(job) {
    ...
    return names.map((n, i) => ({
      name: n,
      hours: Math.round(job.hoursActual * share[i] / 1),
