...
//
// The Electron main process is CommonJS (a top-level `return` after the
// single-instance lock is legal there); the two preload scripts run in a
// browser-ish context with Node's require available.
config.push(
  {
    files: ["electron/**/*.js"],
    languageOptions: { sourceType: "commonjs", globals: { ...globals.node, ...globals.es2024 } },
  },
  {
    files: ["electron/preload.js", "electron/appPreload.js"],
    languageOptions: { sourceType: "commonjs", globals: { ...globals.node, ...globals.browser, ...globals.es2024 } },
  },
);
