...
// None can be added here yet. The canonical block above closes
// `export default [...]` without naming the array, so entries cannot be
// appended after this marker; a self-import does not help because ESLint
// loads this file as `file:///...eslint.config.mjs?mtime=...`, which is a
// separate module instance from the one a plain `./eslint.config.mjs` import
// resolves to. Until the canonical file exports a named array (for example
// `const config = [...]; export default config;` and `config.push(...)` here),
// the relaxations this app needs for the legacy in-browser frontend live as
// directive comments at the top of frontend/app.jsx.
